...
public constant  -- debug level
  NO_DEBUG = 0, DEBUG = 1, VERBOSE = 2, EXTENSIVE = 3

public constant  -- error level
  CRITICAL = 1, ERROR = 2, WARNING = 3, INFO = 4

global integer f_debug = 1    -- debug file
global integer debug_level = NO_DEBUG  -- if set, log_puts and log_printf will write to debug file
global integer error_level = WARNING   -- errors with level <= error_level will be displayed and written to debug file
global integer debug_show_prefix = 0   -- if set, log_puts and log_printf will add a prefix
global integer debug_add_time_stamp = 0   -- if set, log_puts and log_printf will add a time stamp in the prefix
...
public procedure log_puts(sequence msg="", integer level=debug_level,
                          integer show_prefix=debug_show_prefix,
                          integer time_stamp=debug_add_time_stamp)
  sequence dt = date()
  sequence prefix = ""
  if (f_debug = 0) or (debug_level = NO_DEBUG) then return end if
  if (level > debug_level) or length(msg) = 0 then return end if
  if time_stamp then
...
public procedure log_printf(sequence format, object value, integer level=debug_level,
  ...
  log_puts(msg, level, show_prefix, time_stamp)
end procedure

--------------------------------------------------------------------------------

public procedure error_message(sequence msg, integer level=error_level)
  sequence prefix = ""
  sequence title = ""
  integer icon = 0
  integer quit = 0
  ...
  if (level > error_level) or length(msg) = 0 then return end if
  if (f_debug > 0) and (debug_level != NO_DEBUG) then
    puts(f_debug, prefix & msg & "\n")
  end if
ifdef WINDOWS then
  void = message_box(msg, title, icon+MB_OK+MB_APPLMODAL+MB_SETFOREGROUND)
elsifdef LINUX then
  puts(2, prefix & msg & "\n")
end ifdef
  if quit then
    if f_debug then close(f_debug) end if
    ? (1/0)  -- crash to get the state of all variables
    -- abort(1)
  end if
end procedure
...
public function object_dump(object x, integer max_depth=3, integer depth=0)
  integer subSequence
  sequence s
...
--  analyze_object(x, "x")
  ...
  if integer(x) then
    ...
    if depth > max_depth then return "..." end if
    subSequence = 0
    for i=1 to length(x) do
      if sequence(x[i]) then
        subSequence = 1
    ...
    if subSequence = 0 then
      ...
      s = "{" & object_dump(x[1], max_depth, depth+1)
      if length(x) > 1 then
        for i=2 to length(x) do
          s &= ", " & object_dump(x[i], max_depth, depth+1)
        end for
      end if
      s &= "}"
    end if
...
public procedure dump_array(sequence s, sequence name, integer level=debug_level,
                            integer max_depth=3, integer show_prefix=debug_show_prefix,
                            integer time_stamp=debug_add_time_stamp)
  if (f_debug = 0) or (debug_level = NO_DEBUG) then return end if
  if level > debug_level then return end if
  log_printf("%s =\n", {name}, level, show_prefix, time_stamp)
  for i = 1 to length(s) do
    printf(f_debug, "  [%d] %s\n", {i, object_dump(s[i], max_depth)})
  end for
end procedure

--------------------------------------------------------------------------------

-- translator: routine id that translates indexes in understandable names
-- defaults to -1 (no routine provided)
-- example of a conversion routine:
public procedure analyze_object(object x, sequence Name, integer level=debug_level,
                                integer translator=-1, 
                                integer show_prefix=debug_show_prefix,
                                integer time_stamp=debug_add_time_stamp,
                                integer max_depth=10,
                                sequence path={}, integer depth=0,
                                sequence item_prefix="")
-- prints a sequence structure in a human readable way
  ...
  if depth=0 then
    offset = ""
    log_printf("%s =", {Name}, level, show_prefix, time_stamp)
  end if
  for i = 1 to depth-1 do offset &= ".  " end for
  sequence s = ""
  if depth > max_depth then
    -- log_printf("%s%s ...\n", {offset, item_prefix}, level, 0, 0)
    if integer(x) and (translator != -1) then
      integer n = 0
      if length(path) then
        n = path[$]
      end if
      s = call_func(translator, {path, level, n, x})
      log_printf("s[2] = %s\n", {s[2]}, EXTENSIVE)
      log_printf("%s%s %s\n", {offset, item_prefix, s[2]}, level, 0, 0)
    else
      log_printf("%s%s %s\n", {offset, item_prefix, object_dump(x)}, level, 0, 0)
    end if
    return
  end if
  if atom(x) then
    if integer(x) and (translator != -1) then
      ...
      log_printf("%s%s %s\n", {offset, item_prefix, pretty_sprint(x, {1})}, level, 0, 0)
    end if
  else
    ...
    if subSequence = 0 then
      ...
      log_printf("%s%s %s\n", {offset, item_prefix, s}, level, 0, 0)
    else
      if length(s) = 0 then
        log_puts(offset&item_prefix&"\n", level, 0, 0)
      end if
      for i=1 to length(x) do
        if translator != -1 then
          s = call_func(translator, {path, level, i, x[i]})
          log_printf("s[1] = %s\n", {s[1]}, EXTENSIVE)
          item_prefix = sprintf("[%s]", { s[1] })
        else
          item_prefix = sprintf("[%d]", i)
        end if
        analyze_object(x[i], Name, level, translator, 0, 0, max_depth, path&i, depth+1, item_prefix)
      end for
    end if
  end if
end procedure
...
public function hexify(sequence data)
  integer lg = length(data)
  sequence binary = ""
  for i = 1 to lg do
    if data[i] < 0 then data[i] += #100 end if
    binary &= sprintf(" %02x", data[i])
  end for
  return binary[2..$]
end function
...
public procedure hexDump(sequence data, atom offset=0, integer level=debug_level)
  ...
  integer lg = length(data)
  if (f_debug = 0) or (debug_level = NO_DEBUG) then return end if
  if (level > debug_level) or (lg = 0) then return end if
  integer start = floor(offset/16)*16
  ...
  for i = start to stop-1 do
    if (i < offset) or (i >= offset + lg) then
      binary &= "   "
      ascii &= " "
    else
      integer char = data[i-offset+1]
      if char < 0 then char += #100 end if
      binary &= sprintf(" %02x", char)
  ...
  lg = length(binary)
  if lg then
    puts(f_debug, address&sprintf("%-49s", {binary})&ascii&"\n")
  end if
  puts(f_debug, "\n")
  flush(f_debug)
end procedure
...
public function find_key(sequence key, sequence where, integer field=1)
  for i = 1 to length(where) do
    if equal(key, where[i][field]) then
      return i
    end if
  end for
  return 0
...
public function get_key_index(object what, sequence where, sequence key)
  for i = 1 to length(where) do
    if equal(key, where[i][1]) and equal(what, where[i][2])then
      return i
    end if
  end for
  return 0
...
public function get_all_keys(object what, sequence where, sequence key)
  sequence result = {}
  for i = 1 to length(where) do
    if equal(key, where[i][1]) and equal(what, where[i][2])then
...
public function link_c_func(atom dll, sequence name, sequence args, atom result)
  integer handle
...
-- dynamically link a C routine as a Euphoria function
public function link_c_proc(atom dll, sequence name, sequence args)
  integer handle
...
public function link_c_var(atom dll, sequence name)
  atom address
